...
def drain_expected_errors(error_channel, expected_texts):
    while True:
        error = error_channel.poll()
        if error is None:
            return
        ...
        if code not in (linuxcnc.NML_ERROR, linuxcnc.OPERATOR_ERROR):
            log("LinuxCNC message %s: %s" % error)
            continue
        if any(expected_text in text for expected_text in expected_texts):
            log("ignoring expected follow-on error: %s" % text)
            continue
        fail("unexpected LinuxCNC error %s: %s" % error)
...
mdi("G0 X1500.0 Y850.0 Z-600.0 B45.0 C90.0")
assert_close_tuple(
    "safe reposition",
    world_tool_tuple(s),
...
assert_twp_state(False, False, 0, False, 0.0, False)
log("recovery step 3 ok")

mdi("G43.4")
assert_twp_state(False, False, 0, False, 0.0, True)
log("recovery step 4 ok")
...
log("recovery step 5 ok")

mdi("G1 X50.0 F1200.0")
assert_twp_state(True, True, 3, True, 0.0, True)
assert_close_tuple(
    "safe TWP move after recovery",
    world_tool_tuple(s),
    (1500.0, 885.355, -635.355, 45.0, 90.0),
)
log("recovery step 6 ok")
