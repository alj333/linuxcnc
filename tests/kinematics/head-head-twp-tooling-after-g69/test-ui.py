...
def assert_tool_offset_z(status, expected_z):
    status.poll()
    if math.fabs(status.tool_offset[2] - expected_z) > POS_TOL:
        fail(
            "unexpected tool_offset.z: actual=%s expected=%s"
            % (status.tool_offset[2], expected_z)
        )
...
wait_for_pause(
    s,
    e,
    "current tool number changed after G69",
    (1500.0, 850.0, -600.0, 45.0, 90.0),
)
assert_twp_state(False, False, 0, False, 0.0, True)
s.poll()
if s.tool_in_spindle != 1:
    fail("unexpected tool in spindle after M61 Q1: %s" % (s.tool_in_spindle,))
assert_tool_offset_z(s, 0.0)
log("pause 5 ok")
...
wait_for_pause(
    s,
    e,
    "tool length compensation applied after G69",
    (1500.0, 850.0, -600.0, 45.0, 90.0),
)
assert_twp_state(False, False, 0, False, 0.0, True)
assert_tool_offset_z(s, 25.0)
log("pause 6 ok")
...
wait_for_pause(
    s,
    e,
    "tool length compensation cancelled after G69",
    (1500.0, 850.0, -600.0, 45.0, 90.0),
)
assert_twp_state(False, False, 0, False, 0.0, True)
assert_tool_offset_z(s, 0.0)
