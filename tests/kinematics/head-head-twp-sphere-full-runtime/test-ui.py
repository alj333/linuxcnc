...
PASS_CSV = (
    ...
    / "twp-sphere-stage1-t4-passes.csv"
)
RESULT_CSV = (
    ROOT
    / "configs/5th_axis_xyzbc_ssi_tcpc_probe_basic"
    / "twp-sphere-stage1-t4-results.csv"
)
...
def validate_logs(pass_baseline, result_baseline, sphere_center):
    ...
    for index, row in enumerate(pass_rows):
        ...
        assert_scalar("pass campaign", values[1], 2026083101.0, 0.1)
        ...
        assert_scalar("pass B", values[5], START_B, ROTARY_TOL)
        assert_scalar("pass C", values[6], START_C, ROTARY_TOL)
    ...
    values = parse_numeric_row("accepted result row", result_rows[0], 27)
    assert_scalar("result schema", values[0], 1.0, 0.01)
    assert_scalar("result campaign", values[1], 2026083101.0, 0.1)
    assert_scalar("result B", values[2], START_B, ROTARY_TOL)
    assert_scalar("result C", values[3], START_C, ROTARY_TOL)
    assert_scalar("result tool length", values[4], TOOL_LENGTH, 2e-6)
    assert_scalar("result probe offset", values[5], PROBE_OFFSET, 2e-6)

    world_open = values[6:9]
    twp_world = values[9:12]
    world_close = values[12:15]
    ...
    if values[15] > 0.050:
        fail("accepted world closure exceeds 0.050 mm: %.9f" % values[15])
    if values[16] > 0.050:
        fail("accepted transformed TWP error exceeds 0.050 mm: %.9f" % values[16])
    for field, value in zip(("world open", "TWP", "world close"), values[17:20]):
        if value > 0.100:
            fail("%s pair delta exceeds 0.100 mm: %.9f" % (field, value))
    for field, value in zip(("world open", "TWP", "world close"), values[20:23]):
        if not 29.9 <= value <= 30.5:
            fail("%s diameter out of range: %.9f" % (field, value))
    for field, value in zip(("world open", "TWP", "world close"), values[23:26]):
        if value > 0.250:
            fail("%s residual exceeds 0.250 mm: %.9f" % (field, value))
    assert_scalar("result gated edge count", values[26], 24.0, 0.1)
    return values
...
def run_actual_program(sphere_center, start_tcp, tool_w, pass_baseline, result_baseline):
    ...
    try:
        while time.monotonic() < deadline:
            ...
            joints = joint_pose()
            tcp = physical_tcp()
            b_error = abs(joints[3] - START_B)
            c_error = abs(((joints[4] - START_C + 540.0) % 360.0) - 180.0)
            max_b_error = max(max_b_error, b_error)
            ...
            if twp_active and not prior_twp:
                twp_entry_tcp = tcp
                plane_z = vector_scale(dot_vector("headheadkins.tool-frame-w"), -1.0)
    ...
    if max_b_error > ROTARY_TOL or max_c_error > ROTARY_TOL:
        fail(
            "B/C changed during program: max B error %.9g, max C error %.9g"
            % (max_b_error, max_c_error)
        )
    ...
    log(
        "actual TWP sphere program passed: 24 contacts, six passes, "
        "world closure %.6f mm, TWP error %.6f mm"
        % (result_values[15], result_values[16])
    )
    log(
        "preflight projection %.6f mm, closure %.6f mm, final TCP %s"
        % (
            preflight_max_projection,
