...
r: (Reverse):: This is a modifier that indicates that the bit order in the
 following field should be reversed before any other decoding is applied.
 This is intended for absolute encoder formats where the captured field
 significance is opposite to the datasheet field order.
m: (Multi-turn)::
