  ...
  if ((block->m_modes[6] != -1)  && ONCE_M(6)){
	  ...
	  if (IS_USER_MCODE(block,settings,6) && remapped_in_block) {
	      ...
	      CHKS((headhead_tcpc_tool_length_guard_enabled() && headhead_tcpc_is_active()),
		   (_("Cannot change current tool number while TCPC is active; exit TCPC with G49.1 first")));
...
int Interp::convert_tool_change(setup_pointer settings)  //!< pointer to machine settings
{
  ...
  CHKS((headhead_tcpc_tool_length_guard_enabled() && headhead_tcpc_is_active()),
       (_("Cannot change tools while TCPC is active; exit TCPC with G49.1 first")));
  if (settings->tool_change_reject_spindle_on) {
      for (int s = 0; s < settings->num_spindles; s++) {
          CHKS((settings->spindle_turning[s] != CANON_STOPPED),
