...
{
    double b_eff = b_cmd + pinv(haldata->b_zero_offset);
    double c_eff = c_cmd + pinv(haldata->c_zero_offset);
    double c_to_b[3];
    double b_to_tool[3];
    double b_rotated[3];
    double c_frame[3];

    c_to_b[0] = pinv(haldata->nominal_c_to_b_x) + pinv(haldata->cal_c_to_b_x);
    c_to_b[1] = pinv(haldata->nominal_c_to_b_y) + pinv(haldata->cal_c_to_b_y);
    c_to_b[2] = pinv(haldata->nominal_c_to_b_z) + pinv(haldata->cal_c_to_b_z);

    b_to_tool[0] = pinv(haldata->nominal_b_to_tool_x) + pinv(haldata->cal_b_to_tool_x);
    b_to_tool[1] = pinv(haldata->nominal_b_to_tool_y) + pinv(haldata->cal_b_to_tool_y);
    b_to_tool[2] = pinv(haldata->nominal_b_to_tool_z) + pinv(haldata->cal_b_to_tool_z);
