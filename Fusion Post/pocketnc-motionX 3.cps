...
description = "MotionX 5th Axis Post Processor for modified LinuxCNC";
vendor = "MotionX";
...
longDescription = "MotionX 5th Axis Post Processor for modified LinuxCNC";
...
// TCPC (G43.4 / G49.1) active-state tracker. Replaces the legacy M428/M429 modal.
// G43 and G49 are locked out by the control while TCPC is active, so we must
// track state explicitly to gate the enable/disable sequences.
...
function onOpen() {
  ...
  tcpcActive = false; // default to TCPC off at program start
  spindleMin = 500;
  spindleMax = 18000;
  
  var bAxis = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[-95, 95], preference:0});
  var cAxis = createAxis({coordinate:2, table:false, axis:[0, 0, -1], range:[-360, 360], preference:0, cyclic:false});
