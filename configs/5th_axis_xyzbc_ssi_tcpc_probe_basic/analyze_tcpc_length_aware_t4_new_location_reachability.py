#!/usr/bin/env python3
"""Replay the T4 location-2 grid against the frozen q=0 kinematics.
...
RUNNER = REPO_ROOT / "nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt1.ngc"
...
DEFAULT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_REACHABILITY_REPORT.md"
DEFAULT_DETAILS = HERE / "tcpc-length-aware-t4-new-location-2026082701-attempt1-reachability.csv"
...
EXPECTED_SHA256 = {
    ...
    RUNNER: "54bd1e3b5cfc95f44ddbf344693652b68dec920f74649e466d939860fe4a9174",
...
def replay(start_xyz: np.ndarray) -> tuple[np.ndarray, list[reach.Sample], reach.Limits]:
    ...
    poses = reach.grid()
    if len(poses) != 101:
        raise ReachabilityError("canonical T4 grid is not 101 rows")
...
def write_report(
    ...
    lines = [
        ...
        f"- sampled grid/path points: `{len(samples)}` over `101` poses",
...
def main() -> int:
    ...
    try:
        ...
        if args.self_test:
            ...
            print("T4 new-location reachability self-test: PASS")
            return 0
