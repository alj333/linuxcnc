#!/usr/bin/env python3
"""Replay the exact T4 location-2 Attempt-3 grid against frozen q=0 kinematics.
...
RUNNER = REPO_ROOT / "nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt3_recovery.ngc"
...
DEFAULT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_ATTEMPT3_RECOVERY_REACHABILITY_REPORT.md"
DEFAULT_DETAILS = HERE / "tcpc-length-aware-t4-new-location-2026082701-attempt3-recovery-reachability.csv"
...
EXPECTED_SHA256 = {
    ...
    RUNNER: "bf76ab273c76a32046e6f2066f6b865ea8e0a448266cff0399186e262c5a061a",
...
def recovery_grid() -> list[reach.Pose]:
    """Use exact Attempt-3 order: bridge IDs 1..9, 17, 20, then 21..101."""
    return [
        pose for pose in reach.grid()
        if 1 <= pose.slot <= 9 or pose.slot == 17 or 20 <= pose.slot <= 101
    ]
...
def replay(start_xyz: np.ndarray) -> tuple[np.ndarray, list[reach.Sample], reach.Limits]:
    ...
    poses = recovery_grid()
    if len(poses) != 92:
        raise ReachabilityError("T4 Attempt-3 recovery grid is not 92 rows")
