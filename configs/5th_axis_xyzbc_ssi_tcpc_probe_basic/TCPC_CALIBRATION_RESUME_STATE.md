...
- Length-aware model revision `2026082601` is frozen. T4 is the formal
  uninterrupted q0 common-bank validation; T3 is the accepted composite q1
  engineering endpoint. No fit change is pending before cut testing.
- The shared launcher and both TCPC desktop shortcuts now select
  `5th_axis_xyzbc_ssi_tcpc_probe_basic_length_model_validation_2026082601.ini`.
  `/home/cnc5/.linuxcncrc` records the same `LAST_CONFIG`. The configuration
  opens the sealed blank file; it does not automatically start a program.
- T4 equal-76 centered RMS/max improved from the frozen original H0 field
  `0.219602/0.709875 mm` to fresh recalibrated actual
  `0.107589/0.241710 mm`, reductions of `51.0%/66.0%`.
- T3 equal-20 centered RMS/max is `0.103060/0.218333 mm`, compared with
  `0.251775/0.592990 mm` reconstructed without the length-aware correction.
  Physical accuracy evidence currently brackets T3 through T4,
  `128.606729..229.407000 mm`; the software guard domain is wider and is not
  equivalent to physical validation of every length.
- The completed T4 new-location composite contains `101` rows, `28` closures,
  and `808/808` guarded contacts. It is retained only as axis-geometry
  evidence. The equal-pose location/session field delta is
  `0.155952/0.491649 mm` RMS/max. X-oriented indicated span changed
  `-0.095544 mm`; Y-oriented span changed `+0.009149 mm`. These results do not
  prove an X-rail fault and must not be absorbed into global TCPC coefficients.
- Final reports are
  `TCPC_LENGTH_AWARE_T4_ATTEMPT2_CLOSEOUT_REPORT.md`,
  `TCPC_LENGTH_AWARE_T3_ATTEMPT2_RECOVERY_CLOSEOUT_REPORT.md`, and
  `TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_SPATIAL_COMPARISON_REPORT.md`.
  The complete current handoff is
  `TCPC_CALIBRATION_CUT_TEST_HANDOFF_20260828.md`.
- At `2026-08-28T18:11+07:00`, after the operator closed LinuxCNC, no
  LinuxCNC, Probe Basic, milltask, or RTAPI process and no
  `/tmp/linuxcnc.lock` remained. CNC hardware power-down remains
  operator-owned. At the next start, assume no retained homing, tool, TLO,
  coordinate, G43.4, or probe state.
- Next stage is controlled program cut testing. Clean-launch the default
  configuration, home normally, verify the selected tool and live TLO, enter
  G43.4 through the normal guarded sequence, and confirm model ID
  `2026082601` with fault `0` before any test program is started. Program start
  and all machine motion remain operator-owned.

The historical attempt-by-attempt handoffs below are retained as provenance.
They are superseded operationally by this checkpoint.

## Current Handoff - T4 New-Location Attempt-3 Retired At Sequence 1, 2026-08-27

- Attempt 3 stopped itself at `20:24:07 +07` during sequence 1, B0/C0,
  acquisition try 1, pass 1. The sign-aware upper-U contact triggered after
  only `0.039396 mm`, below the `1.000000 mm` travel floor. W/-V/+V traveled
  `3.882667/3.570167/4.271833 mm` normally.
- Three matched raw/mux pulses occurred with the gate closed before U; two
  more gate-closed repeat pulses followed its accepted edge. None of those
  five extras reached the motion input. This is a probe pulse episode, not a
  TCPC geometry result.
- No result/state/model/closure row was accepted. Those four outputs remain
  header-only; contact and gap traces each contain exactly four rejected
  sequence-1 pass-1 rows. Exclude all Attempt-3 traces from calibration.
- Attempt 3 is retired. Do not Resume, restart, append, or reuse it. LinuxCNC
  is enabled, homed, idle, queue-zero, in position and probe-clear with
  Attempt 3 still selected at its derived B0/C0 top-clear point
  `X2501.941317473 Y696.899347649 Z-279.748794997 B0 C0`; counters are
  `835/835/337`.
- Detailed evidence and frozen hashes are in
  `TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_ATTEMPT3_RECOVERY_ZERO_ROW_REPORT.md`.
  A new attempt must use fresh identity/output files and, because zero rows
  were accepted, repeat the entire Attempt-3 topology after T4 reset/reseat
  and qualification.
- The failure evidence is sealed at
  `calibration_runs/20260827_2034_campaign2026082701_t4_new_location_attempt3_recovery_zero_row_false_u_seq1`.
  Its 24-entry inventory verifies, contains no symlink or special file, and
  its root `SHA256SUMS` SHA-256 is
  `510ee53acaf91e8b4cb4e63f20e4eb160fa5f4231d1e371df0ede6430a292b27`.

## Current Handoff - T4 New-Location Attempt-3 Ready For Live Gates, 2026-08-27

- Attempt 2 is retired and remains selected in LinuxCNC. The operator's later
  Cycle Start did not execute a new recovery: read-only status and the edge
  monitor both show LinuxCNC enabled, idle, queue-zero, in position, velocity
  zero, with Attempt 2 still loaded at its rejected sequence-21 pose
  `X2502.405372075 Y698.018973813 Z-279.792014243 B-5 C225`. Attempt-3 outputs
  remain exact header-only files. Do not Resume or restart Attempt 2.
- Fresh recovery identity `2026082701 / mode 37 / attempt 3` is frozen at
  `nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt3_recovery.ngc`,
  SHA-256
  `bf76ab273c76a32046e6f2066f6b865ea8e0a448266cff0399186e262c5a061a`.
- Attempt 3 acquires IDs `1..9,17,20..101`: `92/92/92` summary/state/model
  rows, `30` source-local/bridge closures, and `736/736` contact/gap traces.
  Final canonical ownership is Attempt 1 rows `1..17`, Attempt 2 rows
  `18..20`, and Attempt 3 rows `21..101`; all rejected Attempt-2 row-21
  traces are excluded.
- The exact 92-pose q0 trajectory replay passes `28,345` sampled path points,
  including the new direct B-5/C0 to B-5/C180 transit. Worst configured linear
  margin after reserves is `181.641553 mm`; rotary margins are B `10 deg` and
  C `44 deg`.
- The read-only validator passes compile, static, fresh-header, and mutation
  self-tests. The runner retains one initial M0, no M1, no pre-M0 motion, no
  retry, the 1 mm contact-travel floor, and at most eight matched gate-closed
  diagnostic extras. Every gated G38 and release/state/model guard remains.
- Remaining actions are operator-owned: reset/reseat and qualify T4, return to
  the frozen B0/C0 start
  `X2501.941254485 Y696.899347451 Z-280.866128272 B0 C0`, then explicitly
  clear loading. Loading is not Cycle Start authorization. At the sole M0,
  reconfirm the physical setup and complete a fresh quiet electronic check
  before the operator decides whether to Resume.
- Authoritative instructions and evidence are in
  `TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_ATTEMPT3_RECOVERY_PLAN.md`,
...
- The immutable offline preflight package is
  `calibration_runs/20260827_1907_campaign2026082701_t4_new_location_attempt3_recovery_preflight`.
  Its 104-entry inventory verifies, contains no symlink or special file, and
  its root `SHA256SUMS` SHA-256 is
  `2512d08ce9ef9007a0e233ceaac1505509cef5189866d457f40a3e5e35bc37f5`.
- The operator restored the exact frozen sphere-top start and explicitly
  cleared loading. Pre-load commanded XYZBC was
  `2501.941254485,696.899347451,-280.866128272,0,0`; T4/H4 `229.407000`,
  G43.4, model `2026082601 q=0`, probe-clear, SSI-valid, homed, enabled,
  idle, queue-zero and stationary checks passed. The operator explicitly
  waived the manual deflection test.
- At `19:45 +07`, the exact Attempt-3 runner was loaded. LinuxCNC accepted it
  with lines `0/0/0`, no position change, no motion, and no output write. All
  six outputs remained their frozen one-line headers; the post-load file-only
  preflight passed again. No Cycle Start or Resume command was issued by the
  calibration preparation process. The operator may press Cycle Start once
  to reach the sole M0, where there must be no motion.
- The operator pressed Cycle Start and reached the sole M0 without motion.
  A `30.662 s` read-only quiet qualification passed: `3,025` direct status
  samples and `31` edge-monitor heartbeats, counters unchanged at
  `826/826/333`, zero non-heartbeat events, zero asserted probe/gate/fault
  levels, and zero commanded-position change. Attempt 3 remained selected,
  paused, queue-zero and stationary at the exact frozen start. With the
  operator's stated physical clearance and deflection-test waiver retained,
  the operator was cleared to press Resume once; no Resume command was issued
  by the calibration preparation process.

## Current Handoff - T4 New-Location Attempt-2 Retired, 2026-08-27

- T4 new-location Attempt 1 is retired after 17 accepted canonical rows. Its
  incomplete row-18 prefix ended on four matched raw/mux pulses while the G38
  gate was closed; gated count did not change and no false edge reached
  motion. Preserve Attempt-1 rows `1..17` and exclude all row-18 prefix data.
- The partial is sealed under
  `calibration_runs/20260827_1754_campaign2026082701_t4_new_location_attempt1_partial_gap_abort_seq18`.
  Its 21-entry inventory verifies; root `SHA256SUMS` SHA-256 is
  `2cef1968a26d61cf3f14c6a8807541ce3462f92a8927e6a44e643901234ac6f2`.
- The ready recovery identity is `2026082701 / mode 36 / attempt 2`, model
  `2026082601`, T4/H4 `229.407000 mm`, q=`0`, and `#3032=0.154742`. Runner:
  `nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt2_recovery.ngc`,
  SHA-256
  `c027a0bab19f403e5e625f01fb50d6d050b51188fa0a0885dbaa795035b5c758`.
- Recovery emits sequence IDs `1..9,17..101` exactly: repeated B0 continuity
  data, a repeated B-5/C0 bridge, and all missing rows `18..101`. A valid
  composite uses Attempt-1 rows `1..17` and recovery rows `18..101`. Expected
  recovery counts are `94/94/94` summaries, `29` closures, and `752/752`
  contact/gap traces.
- Only the matched gate-closed diagnostic-extra bound changes from two to
  eight. Raw/mux agreement, exactly one gated edge per successful G38, zero
  gated repeats, released levels, quiet initial baseline, and all live/model
  guards remain mandatory. There is one initial M0, no M1, no pre-M0 motion,
  no later hold, and no whole-pose retry.
- Primary compile/static/preflight/mutation tests pass. Independent file-only
  audit passes 171 checks. The q0 reachability replay passes 28,734 points;
  worst remaining configured-limit margin after reserves is `181.641553 mm`.
  Full evidence is in
...
- The complete pre-run package is sealed at
  ...
  Its 65-entry inventory verifies and root `SHA256SUMS` SHA-256 is
  `bf8230e538399364cbe36d1234b82e7bec656e7a7ff1c24b4f71dcd8e71d8f82`.
- At the last pre-load read, LinuxCNC was active, enabled, homed, idle,
  queue-zero, in position, and stationary at commanded XYZBC
  `2501.941254485,696.899347451,-280.866128272,0,0`. T4/H4, G43.4, model q0,
  spindle stop, live probe clear, and SSI validity all passed. Counters were
  `713/713/225`. The retired Attempt-1 runner remained selected.
- The operator explicitly cleared loading. At `18:19 +07`, LinuxCNC accepted
  and selected the exact recovery runner. Post-load status remained enabled,
  homed, idle, queue-zero, unpaused, in position and stationary at the frozen
  XYZBC. `current/read/motion` lines were `0/0/0`; outputs stayed header-only,
  counters stayed `713/713/225`, and no Cycle Start, Resume, MDI, homing, or
  motion command was issued.
- The operator subsequently pressed Cycle Start once and reached the sole M0
  without motion. A 100 Hz monitor window from `18:21:36` through `18:22:50
  +07` passes for `74.400 s`: only heartbeats, counters fixed at
  `713/713/225`, all probe/gate/fault/motion values clear, and unchanged XYZBC.
  Outputs remain header-only. The workshop is closed and no further laser work
  will occur. Resume is held only for fresh operator confirmation that the
  sphere and nearby fixtures are secured and clear and the post direction
  remains base-to-sphere X+/Y-/Z+.
- The operator confirmed that physical gate and resumed. At `18:48:46 +07`,
  Attempt 2 stopped itself on rejected canonical sequence 21 B-5/C225. The -V
  contact triggered after `0.609798 mm`, below the 1 mm hard travel guard,
  following two matched gate-closed raw/mux pulses. This is a probe pulse
  episode, not a TCPC/model/SSI fault.
- Accepted recovery rows are exactly `1..9,17..20`; their first `104/104`
  contact/gap traces are valid and all three closures pass at
  `0.021536/0.006576/0.026743 mm`. Composite salvage is Attempt-1 canonical
  rows `1..17` plus Attempt-2 canonical rows `18..20`. Exclude all four
  sequence-21 traces; no accepted summary row 21 exists.
- Attempt 2 is retired. Do not Resume, append, restart, or reuse it. The
  controller is idle, queue-zero, in position and probe-clear at
  `X2502.405372075 Y698.018973813 Z-279.792014243 B-5 C225`. Prepare a fresh
  attempt only after probe reset/reseat and new qualification.
- The stopped evidence is sealed at
  ...
  Its 22-entry inventory verifies and root `SHA256SUMS` SHA-256 is
  `466c730dd4732e930f0d97da5ecbae6715374b7855535e62c7d3cf30c8437481`.

## Current Handoff - T4 New Machine-Volume Location Ready, 2026-08-27

- A fresh full T4 transfer acquisition is prepared as
  `2026082701 / mode 35 / attempt 1`, with frozen model `2026082601`, T4/H4
  `229.407000 mm`, and `#3032=0.154742`. The runner is
...
- The runner preserves the sealed T4 Attempt-2 motion and acquisition contract:
  exact `101` rows / `76` unique poses, `28` closures, `808/808` two-pass
  contact/gap traces, one initial M0, no clearance-test holds, no long dwell,
  no whole-pose retry, and the same bounded duplicate-pulse handling. All six
  Attempt-1 outputs are fresh exact header-only files.
- Static validator/self-tests and the full-domain length-model audit pass. The
  exact q=0 location replay sampled `30,653` points and passed; worst remaining
  configured-limit margin after the 2 mm center plus 3 mm path/model reserves
  is `181.641553 mm` on J2 at the updated start recorded below.
- After loading but before program execution, the operator raised Z because
  the initial clearance was too small. The observed commanded change was
  `+2.100000 mm`, and the exact replay was rerun from
  `X2501.941254485 Y696.899347451 Z-280.866128272 B0 C0`. It passes with
  `181.641553 mm` worst remaining margin on J2. Moving more than `2 mm` from
  this updated XYZ invalidates the release until replayed. LinuxCNC remains
  enabled, homed, idle, in position and queue-zero; T4/H4, G43.4, TWP-clear,
  model q0/fault0, spindle, SSI and live probe states all pass.
- At `16:07 +07`, explicit operator clearance was received and the exact new
  runner was loaded successfully. LinuxCNC accepted it and remains idle,
  queue-zero and in position. The load itself changed no XYZBC and issued no
  Cycle Start, Resume, MDI, homing, or motion command. The later Z jog was
  operator-owned. Read/motion lines remain zero; `current_line` may retain the
  completed prior M2 value while idle. All six acquisition outputs remain exact
  header-only files.
- The pre-run evidence is sealed under
  ...
  all entries verify and there are no symlinks or special files.
- The post-load Z adjustment and exact-start replay are sealed separately under
  ...
  all entries verify and there are no symlinks or special files. It supplements
  the original preflight archive and records that the runner remained
  unexecuted after the observed `Z +2.100000 mm` operator move.
- Probe qualification must be repeated: the gate-closed raw/mux counters rose
  from `127/127` after the initial two deflections to `182/182`, while gated
  remained `88`. The last unexplained pulse was at `16:01:34`; none reached
  motion and the probe released. Unless those pulses were deliberate operator
  touches, the prior qualification is invalid. Require continued clear state
  and a fresh 30-second quiet interval at the runner's sole M0, with the laser
  off.
- The physical relocation is approximately X `+1477 mm`, Y `-147 mm`, with a
  small Z-height change. Treat it as a second machine-volume location, not an
  isolated X-rail test. Analysis removes only one independent global XYZ mean
  from each equal-76 field; it does not refit coefficients or infer an axis
  correction table from two locations.
- Authoritative operator order and reachability evidence are in
  `TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_PLAN.md` and
  `TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_REACHABILITY_REPORT.md`.
  Offline closeout uses
...
## Current Handoff - T3 Composite Engineering Pass, 2026-08-27

- The T3 length-aware endpoint is complete as a composite engineering pass.
  Attempt 1 supplies canonical rows `1..22`; recovery identity
  `2026082602 / 34 / 2` supplies canonical rows `23..31` from recovery rows
  `3..11`. Recovery rows 1 and 2 are continuity bridges only. No coordinate
  alignment, refit, or source-data translation is applied.
- The frozen file-only validator and an independent arithmetic audit both pass:
  exact `11/11/11` recovery result/state/model rows, six closures, and `88/88`
  contact/gap traces with no missing or duplicate identity and no accepted bad
  probe point.
- B0 and B-90 cross-attempt bridges are `0.005719/0.013314 mm`; the worst
  reconstructed canonical closure is `0.016482 mm`, all below `0.050 mm`.
- T3 length-aware equal-20-pose RMS/max is `0.103060/0.218333 mm`, versus
  reconstructed length-independent H0 at `0.251775/0.592990 mm`. Positive B,
  negative B, and B0 all improve; maximum individual-pose worsening is
  `0.008584 mm`.
- Four delayed post-contact raw/mux edges occurred across three contacts; none
  reached the gated motion input. All 88 G38 transactions have exact one-edge
  gated deltas, all intervening gaps are quiet, and no trace fault is set.
- Attempt 1's row-23 false W touch remains rejected and excluded. Because this
  accepted field spans a probe reseat, power cycle, and two acquisition IDs, it
  is `COMPOSITE ENGINEERING PASS - NOT FORMAL UNINTERRUPTED T3 RELEASE`. A new
  full 31-row run is needed only if formal same-acquisition release is required.
- Full analysis and file hashes are in
  `TCPC_LENGTH_AWARE_T3_ATTEMPT2_RECOVERY_CLOSEOUT_REPORT.md`. The sealed
  Attempt-1 partial and Attempt-2 preflight archives remain authoritative
  prerequisites.
- The complete read-only evidence package is
  `calibration_runs/20260827_1503_campaign2026082602_t3_length_aware_attempt2_recovery_complete`.
  Its root `SHA256SUMS` contains 186 entries and hashes to
  `91296bbd468f5b5f67c0722c1142ce0a0edbf3bedd4bd4579aa6f53f68215bca`.
- Final read-only live audit: LinuxCNC remains running under the dedicated
  validation INI, enabled, all five joints homed, idle at `M2`, queue zero and
  in position. Commanded XYZBC is
  `1024.841254075,843.999347176,-380.410713136,0,0`; actual XYZBC is
  `1024.841298569,843.999347382,-380.410713265,-0.000010315,-0.000023181`.
  T3/H3 and G43.4 remain active; model `2026082601` is valid at `q=1`, fault
  `0`. Spindle, probe, gate, abnormal-pulse, fault-pause, and SSI-invalid
  signals are clear. The retained `probe_tripped=True` status is the last
  successful G38 result; live probe value is clear and probing is false.
- No further motion is required. It is clear for the operator to close
  LinuxCNC cleanly. Current physical accuracy evidence covers only T3 through
  T4 (`128.606729..229.407000 mm`). The next physical gate is the planned
  dial-gauge endpoint near `425..430 mm`; a second table position remains the
  later machine-volume separation test.

## Current Handoff - T3 Attempt-2 Recovery Ready, 2026-08-27

- T3 Attempt 1 is retired after accepting canonical rows `1..22`. Its
  sequence-23 B-90/C90 second-pass W touch occurred at `2.543169 mm` instead
  of approximately `5 mm`, producing `2.463369 mm` pass-center disagreement.
  The quality guard rejected it before writing an accepted row. Continued
  gate-closed pulses and the operator-observed flashing identify the known
  probe/receiver fault, not a TCPC displacement.
- The partial is sealed read-only under
...
- Machine power was then lost. LinuxCNC/HAL exited after the Mesa read error;
  no controller process or `/tmp/linuxcnc.lock` remains. Do not infer position
  or homing from the terminated session.
- The current runner is the separate recovery identity
  `2026082602 / 34 / 2`:
  `nc_files/calibration/tcpc_length_aware_t3_validation_2026082601_attempt2_recovery.ngc`,
  SHA-256
  `1924e4af8be964a29442f23903e3566daceb4e65dde0e334bd595ba2dcb31294`.
  It has 11 rows, 6 closures, 88/88 contact/gap traces, one initial M0, and no
  whole-pose retry.
- Recovery row 1 hard-checks B0/C0 against Attempt-1 row 16; row 2 hard-checks
  B-90/C0 against Attempt-1 row 22. Both limits are `0.050 mm`. Recovery rows
  3-11 supply canonical rows 23-31 without editing or appending Attempt 1.
- Validator `validate_tcpc_length_aware_t3_attempt2_recovery.py` SHA-256 is
  `cb8960567f39d08bf0ef303110e49910fe50f1d9bb828b200803ba4a6ebe47a1`.
  Python compile, static/preflight, mutation self-test, isolated RS274 parse,
  and independent review pass; all six recovery outputs are header-only.
- The complete read-only recovery pre-run package is
  `calibration_runs/20260827_1416_campaign2026082602_t3_length_aware_attempt2_recovery_preflight`.
  Its root `SHA256SUMS` has 120 entries and SHA-256
  `1dc155d08c27272d2755a4e63b5e7707cd8846aabe5712aa6e23694e9b3f4d77`;
  it includes the sealed Attempt-1 preflight, partial data, and T4 prerequisite.
- Authoritative operator order is
  `TCPC_LENGTH_AWARE_T3_ATTEMPT2_RECOVERY_PLAN.md`. After machine power is
  restored: clean-launch the dedicated validation INI, home, reseat T3, apply
  T3/H3 and G43.4, return to B0/C0 3-5 mm above the sphere, qualify the probe,
  and load only the exact recovery runner. All motion remains operator-owned.
- A successful composite is sufficient for this engineering verification
  stage. It is not a formal uninterrupted T3 release; any new interruption or
  reseat retires recovery Attempt 2.

## Current Handoff - T3 Length-Aware Attempt 1 Ready, 2026-08-27

- At `2026-08-27 11:31 +07`, LinuxCNC was clean-launched with
  `5th_axis_xyzbc_ssi_tcpc_probe_basic_length_model_validation_2026082601.ini`.
  The launch checkpoint is machine-off, idle, unhomed, stationary, and
  unpaused with queue zero, tool 0, zero tool offset, and the spindle stopped.
  Only the configured `blank.ngc` is loaded; the T3 runner has not been loaded
  or started, and no homing, MDI, or motion command was issued.
- The length-model component is present and configured with matching live and
  expected ID `2026082601`. Its startup fault `5` / invalid state is expected
  while tool 0 has zero active length; T3/H3 must be selected and `G43 H3`
  applied before `G43.4` under the operator sequence.
- The completed T4 Attempt 2 is accepted as the `q=0` prerequisite without
  retuning. Reproducible closeout is
  `TCPC_LENGTH_AWARE_T4_ATTEMPT2_CLOSEOUT_REPORT.md`, SHA-256
  `6e33d171d0ab791ff7fd64ae497562c8fe2b282779712180d78b5c2703ebfa45`;
  its analyzer SHA-256 is
  `7371444295d66b6fcd7e75a77689726a46bd1a494c51a272fa246bcc95554b08`.
  Actual equal-76 RMS/max are `0.107589/0.241710 mm` against the frozen
  `0.107256/0.247250 mm` prediction. T4 validates the common bank only.
- The next physical gate is the untouched T3 endpoint, identity
  `2026082602 / 33 / 1`, model ID `2026082601`, T3/H3
  `128.606729 mm`, `q=1`, and `#3032=0.117658`. The runner is
  `nc_files/calibration/tcpc_length_aware_t3_validation_2026082601_attempt1.ngc`,
  SHA-256
  `d6158b9ff91f5fa73a11071d314c64a442d6747f6758587415ece7c867e53bd6`.
- The runner preserves the exact qualified T3 31-row/20-pose path and 14
  closures, uses the T4-proven bounded duplicate-pulse transaction layer, has
  one initial M0, no later hold, and no whole-pose retry. A clean run must
  produce exactly `31/31/31` result/state/model-state rows, `14` closures, and
  `248/248` contact/gap traces.
- Final offline validation passes: Python compile, full static and metric
  mutation suite, full-domain model audit, all 60 members of the immutable T4
  prerequisite archive, exact fresh output headers, and isolated in-tree
  `bin/rs274 -g`. Validator
  `validate_tcpc_length_aware_t3_attempt1.py` SHA-256 is
  `88ddd9a4ead0d5a461cb7de7caa919cb878e0dcaf9dcf7633902abd86a8fbdae`;
  `TCPC_LENGTH_AWARE_T3_ATTEMPT1_PREFLIGHT_REPORT.md` SHA-256 is
  `dc09e6df1b77426fcdb1530fda1c146cec831758a37480450a154e7e208523d0`.
- The complete read-only pre-run package is
  `calibration_runs/20260827_1119_campaign2026082602_t3_length_aware_attempt1_preflight`.
  Its root `SHA256SUMS` has 87 entries and SHA-256
  `3840f95208da9d59b282da5d8d732e6cf8280cf0da47761f0b2bdf46fec56f84`;
  it includes the complete independently verified T4 completion archive.
  There are no symlinks or special files. The final independent T3 audit has
  no open blocker or medium finding.
- The preflight explicitly binds current `headheadkins.c`, runtime
  `headheadkins.so`, and the probe-counter HAL to the exact artifacts used in
  T4 Attempt 2. Every T3 model row must match the offline `q=1` D vector and
  total `H0+S+D` vector.
- Post-run acceptance uses both absolute `0.120/0.280 mm` RMS/max ceilings and
  same-acquisition improvement gates against reconstructed H0. Both B signs
  must improve, B0 may worsen by no more than `0.010 mm`, no unique pose may
  worsen by more than `0.050 mm`, and every closure remains limited to
  `0.050 mm`. A T3 pass supports only the T3-to-T4 length bracket, not the
  `100..430 mm` software envelope or production promotion.
- Authoritative operator order is
  `TCPC_LENGTH_AWARE_T3_ATTEMPT1_PLAN.md`. On the next session, clean-launch
  the dedicated length-model validation INI, home, install T3 in its keyed
  orientation, apply T3/H3 before G43.4, position at B0/C0 3-5 mm above the
  secured sphere, perform the two manual deflections and quiet checks, then
  load the exact runner. Cycle Start and all motion remain with the operator.
...
- At that historical checkpoint the working tree contained the prepared
  calibration changes and the owner's then-uncommitted tool-table work. Those
  files are included in the 2026-08-28 calibration closeout commit. Do not use
  a tree from before that checkpoint to reconstruct the recorded campaign.
