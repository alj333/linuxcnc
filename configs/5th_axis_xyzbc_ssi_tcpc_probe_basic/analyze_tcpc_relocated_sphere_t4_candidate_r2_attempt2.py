#!/usr/bin/env python3
"""Preflight and validate campaign-04 T4 R2 loaded-candidate attempt 2.

This tool has no T3 data input. It performs offline file, fit-identity, G-code,
pose-grid, attempt-isolation, configured-limit, and result-contract checks only.
"""
...
PROGRAM = REPO_ROOT / "nc_files/calibration/tcpc_relocated_sphere_t4_candidate_r2_attempt2_verification.ngc"
RESULTS = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt2-results.csv"
STATE = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt2-state.csv"
CLOSURES = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt2-closures.csv"
ATTEMPT1_PROGRAM = REPO_ROOT / "nc_files/calibration/tcpc_relocated_sphere_t4_candidate_r2_verification.ngc"
ATTEMPT1_ANALYZER = HERE / "analyze_tcpc_relocated_sphere_t4_candidate_r2.py"
ATTEMPT1_RESULTS = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt1-results.csv"
ATTEMPT1_STATE = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt1-state.csv"
ATTEMPT1_CLOSURES = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt1-closures.csv"
ATTEMPT1_ARCHIVE = (
    HERE
    / "calibration_runs/20260825_1412_campaign04_t4_candidate_r2_attempt1_partial_closure_stop"
)
ATTEMPT1_ARCHIVE_SUMS = ATTEMPT1_ARCHIVE / "SHA256SUMS"
ATTEMPT1_ARCHIVE_PROGRAM = ATTEMPT1_ARCHIVE / "tcpc_relocated_sphere_t4_candidate_r2_verification.ngc"
...
DEFAULT_PREFLIGHT_REPORT = HERE / "TCPC_RELOCATED_SPHERE_T4_CANDIDATE_R2_ATTEMPT2_PREFLIGHT_REPORT.md"
DEFAULT_RESULT_REPORT = HERE / "TCPC_RELOCATED_SPHERE_T4_CANDIDATE_R2_ATTEMPT2_REPORT.md"
DEFAULT_REACH_DETAILS = HERE / "tcpc-relocated-sphere-t4-candidate-r2-attempt2-reachability.csv"
...
EXPECTED_PROGRAM_SHA256 = "6421c2f8cb8c12a7e4d8ace98f956e4270974482058815609cce9b5f22dbea86"
EXPECTED_ATTEMPT1_PROGRAM_SHA256 = "a1358c407399ad3606a5a2a449cc973cd39c6ea705233c1f87fdfc0dcb45b7f4"
EXPECTED_ATTEMPT1_ANALYZER_SHA256 = "821e06d288b7b7dd2917cb32de6d06c2c418acf8d1f133e42d3d000ffe5513fa"
EXPECTED_ATTEMPT1_RESULTS_SHA256 = "b96051b207c8c90ae16e1850420e540f9ff9c1fa565f2840ac949f3af0fbb7dc"
EXPECTED_ATTEMPT1_STATE_SHA256 = "89eb547fc08899b75c8b66ee1744dca64eb5922e87a30b298c8a9591e5cf35a6"
EXPECTED_ATTEMPT1_CLOSURES_SHA256 = "8d9a8abfa2ed498d69949dc0f9288d2f42fb3094bfcba6f02ecf3095e4b0a415"
EXPECTED_ATTEMPT1_ARCHIVE_SUMS_SHA256 = "2026776b2b3a1b7b98fc74af2881fe99b2498ddbb7aa0899f8033977ef8156a0"
...
def validate_attempt1_provenance() -> None:
    """Prove that attempt 2 cannot consume or overwrite attempt-1 evidence."""
    require_hash(ATTEMPT1_ARCHIVE_SUMS, EXPECTED_ATTEMPT1_ARCHIVE_SUMS_SHA256)
    listed: dict[str, str] = {}
    for line_number, line in enumerate(
        ATTEMPT1_ARCHIVE_SUMS.read_text(encoding="ascii").splitlines(), 1
    ):
        match = re.fullmatch(r"([0-9a-f]{64})  (.+)", line)
        if match is None:
            raise CandidateError(f"attempt-1 archive checksum line {line_number} is malformed")
        digest, relative_text = match.groups()
        relative = Path(relative_text)
        if relative.is_absolute() or ".." in relative.parts or relative_text == "SHA256SUMS":
            raise CandidateError("attempt-1 archive checksum has an unsafe member")
        if relative_text in listed:
            raise CandidateError("attempt-1 archive checksum has a duplicate member")
        listed[relative_text] = digest

    actual_files: set[str] = set()
    for member in ATTEMPT1_ARCHIVE.rglob("*"):
        if member.is_symlink():
            raise CandidateError(f"attempt-1 archive contains a symlink: {member}")
        if member.is_file() and member != ATTEMPT1_ARCHIVE_SUMS:
            actual_files.add(member.relative_to(ATTEMPT1_ARCHIVE).as_posix())
    if set(listed) != actual_files:
        raise CandidateError("attempt-1 archive members differ from its sealed checksum set")
    for relative_text, digest in listed.items():
        require_hash(ATTEMPT1_ARCHIVE / relative_text, digest)

    for live_path, archived_name, digest in (
        (ATTEMPT1_PROGRAM, "tcpc_relocated_sphere_t4_candidate_r2_verification.ngc", EXPECTED_ATTEMPT1_PROGRAM_SHA256),
        (ATTEMPT1_ANALYZER, "analyze_tcpc_relocated_sphere_t4_candidate_r2.py", EXPECTED_ATTEMPT1_ANALYZER_SHA256),
        (ATTEMPT1_RESULTS, "tcpc-relocated-sphere-t4-candidate-r2-attempt1-results.csv", EXPECTED_ATTEMPT1_RESULTS_SHA256),
        (ATTEMPT1_STATE, "tcpc-relocated-sphere-t4-candidate-r2-attempt1-state.csv", EXPECTED_ATTEMPT1_STATE_SHA256),
        (ATTEMPT1_CLOSURES, "tcpc-relocated-sphere-t4-candidate-r2-attempt1-closures.csv", EXPECTED_ATTEMPT1_CLOSURES_SHA256),
    ):
        require_hash(live_path, digest)
        archived_path = ATTEMPT1_ARCHIVE / archived_name
        require_hash(archived_path, digest)
        if live_path.read_bytes() != archived_path.read_bytes():
            raise CandidateError(f"live attempt-1 artifact differs from sealed archive: {live_path}")


def python_function_text(text: str, function_name: str) -> str:
    tree = ast.parse(text)
    matches = [
        node
    ...
    if len(matches) != 1 or matches[0].end_lineno is None:
        raise CandidateError(f"cannot isolate statistical function {function_name}")
    lines = text.splitlines()
    node = matches[0]
    return "\n".join(lines[node.lineno - 1 : node.end_lineno])


def validate_statistical_gate_derivation() -> None:
    expected_limits = (0.120, 0.280, 0.120, 0.280, 0.050, 0.120, 0.075)
    actual_limits = (
        UNIQUE_RMS_LIMIT_MM,
        UNIQUE_MAX_LIMIT_MM,
        RAW_RMS_LIMIT_MM,
        RAW_MAX_LIMIT_MM,
        PATTERN_RMS_LIMIT_MM,
        PATTERN_MAX_LIMIT_MM,
        MAX_ROW_WORSENING_MM,
    )
    if actual_limits != expected_limits:
        raise CandidateError("attempt-2 statistical thresholds differ from attempt 1")
    attempt1_text = ATTEMPT1_ANALYZER.read_text(encoding="ascii")
    attempt2_text = Path(__file__).resolve().read_text(encoding="ascii")
    for function_name in (
        ...
        "evaluate_result_gates",
    ):
        if python_function_text(attempt2_text, function_name) != python_function_text(
            attempt1_text, function_name
        ):
            raise CandidateError(
                f"attempt-2 statistical contract differs in {function_name}"
            )
...
def validate_attempt2_runner_derivation(text: str) -> None:
    attempt1_token = "tcpc-relocated-sphere-t4-candidate-r2-attempt1"
    attempt2_token = "tcpc-relocated-sphere-t4-candidate-r2-attempt2"
    attempt1_selector = "#727 = 1.0            (candidate attempt ID; increment after an interrupted run)"
    attempt2_selector = "#727 = 2.0            (candidate attempt ID; increment after an interrupted run)"
    if text.count(attempt2_token) != 5 or attempt1_token in text:
        raise CandidateError("attempt-2 runner output namespace is not exact or is contaminated by attempt 1")
    if text.count(attempt2_selector) != 1 or attempt1_selector in text:
        raise CandidateError("attempt-2 runner selector is not exactly #727=2")
    normalized = text.replace(attempt2_token, attempt1_token)
    normalized = normalized.replace(attempt2_selector, attempt1_selector, 1)
    attempt1_text = ATTEMPT1_PROGRAM.read_text(encoding="ascii")
    archived_text = ATTEMPT1_ARCHIVE_PROGRAM.read_text(encoding="ascii")
    if attempt1_text != archived_text:
        raise CandidateError("active and sealed attempt-1 runners differ")
    if normalized != attempt1_text:
        raise CandidateError(
            "attempt-2 runner differs from exact attempt 1 outside output paths and #727"
        )
    validate_r1_template_derivation(attempt1_text)
...
def validate_program(expected_pins: dict[str, float]) -> tuple[list[reach.Pose], str]:
    ...
    required = (
        ...
        "tcpc-relocated-sphere-t4-candidate-r2-attempt2-results.csv",
        "tcpc-relocated-sphere-t4-candidate-r2-attempt2-state.csv",
        "tcpc-relocated-sphere-t4-candidate-r2-attempt2-closures.csv",
        "#539 = -1.0",
    ...
    validate_attempt2_runner_derivation(text)
...
def candidate_run_spec() -> campaign.RunSpec:
    return campaign.RunSpec(
        "T4 loaded-candidate verification attempt 2",
        4,
        MODE,
...
def write_preflight_report(
    ...
    lines = [
        "# T4 R2 Loaded-Candidate Attempt 2 Preflight",
        "",
        ...
        "- sealed attempt-1 partial archive and live attempt-1 artifacts unchanged: `PASS`",
        "- row-count, collapse, residual, threshold, and statistical-gate functions are exact attempt-1 analyzer derivations: `PASS`",
        ...
        "- runner is exact attempt 1 with only #727=2 and attempt-2 output paths, and attempt 1 remains the audited R1-derived program: `PASS`",
...
def write_result_report(
    ...
    lines = [
        "# T4 R2 Loaded-Candidate Attempt 2 Report",
        "",
...
def offline_contract(
    ...
    validate_attempt1_provenance()
    validate_statistical_gate_derivation()
...
def self_test() -> None:
    ...
    broken_derivation = PROGRAM.read_text(encoding="ascii").replace(
        "#707 = 101.0", "#707 = 100.0", 1
    )
    try:
        validate_attempt2_runner_derivation(broken_derivation)
    except CandidateError:
        pass
    else:
        raise AssertionError("attempt-2 runner motion-contract mutation was accepted")
