...
Current calibration stage: length-aware model revision `2026082601` passed its
complete physical T4 Attempt 2 at the reference length (`q=0`). Attempt 1 is a
preserved 36-pose partial and must not be resumed or appended. Use
`TCPC_LENGTH_AWARE_T4_ATTEMPT2_PLAN.md` and the current checkpoint in
`TCPC_CALIBRATION_RESUME_STATE.md`. The Attempt 1 plan, earlier T3 R2-transfer
closeout, and all older campaign handoffs are historical evidence, not the
current operator order.

Attempt 2 completed `101` poses, `28` closures, and `808/808` guarded contact
and gap transactions, then passed formal post-shutdown validation. Fifteen
matching raw/mux extra pulses were filtered while exactly 808 edges reached
motion. LinuxCNC is closed. This validates the common correction bank only;
T3 differential-bank verification and later long-tool dial-gauge validation
...
The current physical gate is the frozen T4 campaign `2026082602`, mode `32`,
attempt `2`. Its exact operator order is in
`TCPC_LENGTH_AWARE_T4_ATTEMPT2_PLAN.md`. Attempt 2 accepts at most two matching
raw/mux duplicate edges around a successful contact only when exactly one
gated edge reaches motion and the probe releases cleanly; all extras remain
logged. T4 tests the common bank at `q=0`. T3 and the longer-tool dial-gauge
endpoint remain separate later gates.
...
The long probe is represented by the current uncommitted T4/H4 tool-table row.
