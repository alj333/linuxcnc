...
the planned dial-gauge test near `425..430 mm`; a later second table location
is still needed to separate rotary structure from machine-volume rail errors.

After completion, LinuxCNC remained enabled, homed, idle at `M2`, in position
at B0/C0 with queue zero. T3/H3 and G43.4 remained active; model
`2026082601` was valid at `q=1`, fault `0`, and all spindle, live probe,
fault-pause, abnormal-level, and SSI-invalid signals were clear. No further
machine motion is required for this stage.
