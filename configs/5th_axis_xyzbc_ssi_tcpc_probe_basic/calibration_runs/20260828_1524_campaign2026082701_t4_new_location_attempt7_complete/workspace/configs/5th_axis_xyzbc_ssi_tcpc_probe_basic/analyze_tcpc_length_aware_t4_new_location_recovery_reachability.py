#!/usr/bin/env python3
"""Replay the T4 location-2 recovery grid against frozen q=0 kinematics.
...
RUNNER = REPO_ROOT / "nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt2_recovery.ngc"
...
DEFAULT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_RECOVERY_REACHABILITY_REPORT.md"
DEFAULT_DETAILS = HERE / "tcpc-length-aware-t4-new-location-2026082701-attempt2-recovery-reachability.csv"
...
START_XYZ = np.array([2501.941254485, 696.899347451, -280.866128272])
...
EXPECTED_SHA256 = {
    ...
    RUNNER: "c027a0bab19f403e5e625f01fb50d6d050b51188fa0a0885dbaa795035b5c758",
...
def estimated_center(start_xyz: np.ndarray) -> np.ndarray:
    # At B0, W=(0,0,-1); the intended top-clear point is center-W*R.
    return start_xyz - np.array([0.0, 0.0, reach.TOP_CLEAR_RADIUS])


def recovery_grid() -> list[reach.Pose]:
    """Keep bridge IDs 1..9 and 17 plus missing canonical IDs 18..101."""
    return [
        pose for pose in reach.grid()
        if 1 <= pose.slot <= 9 or 17 <= pose.slot <= 101
    ]
...
def replay(start_xyz: np.ndarray) -> tuple[np.ndarray, list[reach.Sample], reach.Limits]:
    ...
    poses = recovery_grid()
    if len(poses) != 94:
        raise ReachabilityError("T4 recovery grid is not 94 rows")
...
def summarize(samples: list[reach.Sample], limits: reach.Limits) -> dict[str, object]:
    ...
    reserve = CENTER_ALLOWANCE_MM + PATH_MODEL_ALLOWANCE_MM
    minimum_nominal = min(
...
def write_report(
    ...
    lines = [
        ...
        f"- conservative center estimate: `X{center[0]:.9f} Y{center[1]:.9f} Z{center[2]:.9f}` mm",
        f"- center estimate rule: start Z minus top-clear radius `{reach.TOP_CLEAR_RADIUS:.6f} mm`",
        f"- sampled grid/path points: `{len(samples)}` over `94` recovery poses",
        f"- center uncertainty / path-model reserve: `{CENTER_ALLOWANCE_MM:.3f} / {PATH_MODEL_ALLOWANCE_MM:.3f} mm`",
...
def main() -> int:
    ...
    try:
        ...
        if args.self_test:
            assert np.allclose(
                center,
                [2501.941254485, 696.899347451, -303.711386272],
                atol=1e-9,
            )
            assert summary["passed"]
            assert [pose.slot for pose in recovery_grid()] == [
                *range(1, 10), *range(17, 102)
            ]
            print("T4 new-location recovery reachability self-test: PASS")
            return 0
