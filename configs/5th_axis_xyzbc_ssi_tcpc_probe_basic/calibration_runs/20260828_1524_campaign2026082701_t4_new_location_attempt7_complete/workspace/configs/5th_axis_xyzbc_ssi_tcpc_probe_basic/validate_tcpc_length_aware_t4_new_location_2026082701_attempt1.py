#!/usr/bin/env python3
"""Offline preflight and acquisition validation for T4 location 2."""
...
PROGRAM = REPO_ROOT / "nc_files/calibration/tcpc_length_aware_t4_new_location_2026082701_attempt1.ngc"
...
OUTPUT_BASE = HERE / "tcpc-length-aware-t4-new-location-2026082701-attempt1"
...
DEFAULT_PREFLIGHT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_PREFLIGHT_REPORT.md"
DEFAULT_STATIC_PREFLIGHT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_STATIC_PREFLIGHT_REPORT.md"
DEFAULT_RESULT_REPORT = HERE / "TCPC_LENGTH_AWARE_T4_NEW_LOCATION_2026082701_VALIDATION_REPORT.md"
...
SEALED_SHA256 = {
    ...
    PROGRAM: "54bd1e3b5cfc95f44ddbf344693652b68dec920f74649e466d939860fe4a9174",
...
SPEC = campaign.RunSpec(
    "T4 length-aware q=0 new-location transfer",
    TOOL,
    MODE,
    TOOL_LENGTH_MM,
    PROBE_OFFSET_MM,
...
def offline_contract(*, header_only: bool, parser_preview: bool) -> None:
...
def center_metrics(centers: dict[int, np.ndarray]) -> dict[str, float]:
    ...
    return {
        ...
        "transfer_pass": float(
            unique_rms <= RMS_LIMIT_MM and unique_max <= MAX_LIMIT_MM
        ),
    }
...
def validate_completed_run() -> tuple[dict[str, float], np.ndarray, dict[str, int]]:
    offline_contract(header_only=False, parser_preview=False)
...
def write_preflight_report(path: Path, *, parser_preview: bool) -> None:
    parser_evidence = (
        "- parser: standalone in-tree `bin/rs274 -g` passed under an isolated temporary HOME"
        if parser_preview
        else "- parser: deferred; validate by loading this exact file in the already-running dedicated controller"
    )
    controller_evidence = (
        ...
        if parser_preview
        else "- controller-process gate: LinuxCNC may remain active; standalone rs274 was deliberately not launched"
    )
    lines = [
        f"# TCPC Length-Aware T4 New-Location {'Preflight' if parser_preview else 'Static Preflight'}",
        "",
        "Status: `PASS`",
        "",
    ...
    lines.extend(
        [
            ...
            "## Fresh Outputs",
            "",
            "All six new-location attempt-1 files are exact header-only ASCII files:",
            "",
...
def write_result_report(
    ...
    lines = [
        "# TCPC Length-Aware T4 New-Location Result",
        "",
        "Acquisition validity: `VALID`",
        f"Frozen-model T4 transfer: `{'PASS' if metrics['transfer_pass'] else 'FAIL'}`",
        "",
        f"- campaign / mode / attempt: `{CAMPAIGN} / {MODE} / {ATTEMPT}`",
        ...
        f"- transfer limits: equal-unique RMS `<={RMS_LIMIT_MM:.3f} mm`, max `<={MAX_LIMIT_MM:.3f} mm`",
        "",
        "All acquisition identities, poses, T4 tool/TLO values, model snapshots, q=0 differential bank, empirical vectors/norms/caps, transaction counters/flags, geometry gates, and closure mappings passed.",
        "",
        "A transfer FAIL remains valid location evidence; it does not reclassify a structurally and electrically valid acquisition as corrupt.",
        "",
...
def self_test() -> None:
    offline_contract(header_only=True, parser_preview=False)
    ...
    canonical = CANONICAL_PROGRAM.read_text(encoding="ascii")
    mutations = {
        "campaign": text.replace("#715 = 2026082701.0", "#715 = 2026082702.0", 1),
...
def parse_args() -> argparse.Namespace:
    ...
    group.add_argument("--static-preflight", action="store_true", help="run all preflight checks except standalone rs274")
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            return 0
        if args.preflight or args.static_preflight:
            parser_preview = args.preflight
            offline_contract(header_only=True, parser_preview=parser_preview)
            report = args.report or (
                DEFAULT_PREFLIGHT_REPORT
                if parser_preview
                else DEFAULT_STATIC_PREFLIGHT_REPORT
            )
            write_preflight_report(report, parser_preview=parser_preview)
            label = "preflight" if parser_preview else "static preflight"
            print(f"TCPC length-aware T4 {label}: PASS")
        else:
            ...
            print("TCPC length-aware T4 new-location acquisition validation: PASS")
        ...
        print(f"TCPC length-aware T4 new-location validation: FAIL: {exc}", file=sys.stderr)
        return 1
