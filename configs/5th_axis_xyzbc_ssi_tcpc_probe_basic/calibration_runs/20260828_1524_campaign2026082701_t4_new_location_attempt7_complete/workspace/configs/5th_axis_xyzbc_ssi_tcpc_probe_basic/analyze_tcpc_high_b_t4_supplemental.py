#!/usr/bin/env python3
"""Validate and summarize the isolated T4 high-B sphere supplemental.

This analyzer is deliberately offline.  It reads the dedicated mode-21
...
DEFAULT_RESULTS = SCRIPT_DIR / "tcpc-high-b-t4-supplemental-results.csv"
DEFAULT_STATE = SCRIPT_DIR / "tcpc-high-b-t4-supplemental-state.csv"
DEFAULT_CLOSURES = SCRIPT_DIR / "tcpc-high-b-t4-supplemental-closures.csv"

SCHEMA_VERSION = 1
CAMPAIGN_ID = 2026082402
STAGE_MODE = 21
ATTEMPT_ID = 1
TOOL_NUMBER = 4
TOOL_LENGTH_MM = 229.407000
CALIBRATION_OFFSET_MM = 0.154742
...
MAX_CENTER_CORRECTION_MM = 2.000
...
def parse_sample(
    ...
    if parse_int(raw, "live_tool_number", line) != TOOL_NUMBER:
        raise DataQualityError(f"line {line}: supplemental requires live T4")
    tlo_names = (
        "expected_tool_length_mm",
        "motion_tooloffset_z_mm",
        "halui_tool_length_offset_z_mm",
...
def format_report(
    ...
    lines = [
        "TCPC high-B T4 supplemental validation",
