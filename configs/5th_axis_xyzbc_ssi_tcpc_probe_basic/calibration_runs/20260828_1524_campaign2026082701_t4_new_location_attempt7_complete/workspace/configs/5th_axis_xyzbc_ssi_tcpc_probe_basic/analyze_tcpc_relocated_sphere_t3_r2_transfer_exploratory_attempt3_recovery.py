#!/usr/bin/env python3
"""Validate and score the three-source T3 R2-transfer exploratory acquisition.
...
terminal pre-probe gap burst at sequence 23. Attempt 3 owns sequences 23-31 in
fresh files. The three sources are independently validated before their
...
A3_SEQUENCES = tuple(range(23, 32))
...
DEFAULT_PREFLIGHT_REPORT = (
    HERE / "TCPC_RELOCATED_SPHERE_T3_R2_TRANSFER_EXPLORATORY_ATTEMPT3_RECOVERY_PREFLIGHT_REPORT.md"
)
DEFAULT_RESULT_REPORT = (
    HERE / "TCPC_RELOCATED_SPHERE_T3_R2_TRANSFER_EXPLORATORY_THREE_SOURCE_REPORT.md"
)
...
EXPECTED_HASHES = {
    ...
    A3_PROGRAM: "9db6d03a12085928fb3fe9eacea203e240ee5d52274f5cb04e23d2e981fb18ed",
...
HEADER_HASHES = {
    A3_RESULTS: "9785983d8f89a4955082aa04d8a9e16bf2e2bdc00caccb4cd19f66e545416e93",
    A3_STATE: "ac9e7ddd425e187444dd4ee339466a8e1713ca6e7104ccc76eba6076281427c7",
    A3_CLOSURES: "1f2e125d08ab2a0ea5d2210577c4a593f8cea1fc8cc348f67e3ed2a4a987437f",
    A3_TRACE: "df95e36f729b7bc1e1cef54bf4490ef8530f2e74d52e50671a4c452062c6bbe8",
    A3_GAP_TRACE: "e8e24f1617d5eb0bf637bdadc42f052d7e96130e808761ab07410cdb85e0d6e2",
}
...
def validate_a3_program() -> None:
    require_hash(A3_PROGRAM, EXPECTED_HASHES[A3_PROGRAM])
    text = A3_PROGRAM.read_text(encoding="ascii")
    lines = text.splitlines()
    if len(lines) != 1498 or max(map(len, lines)) > 225:
        raise CompositeError("A3 runner line-count/maximum-line contract changed")
    if text.count("\nM0\n") != 1:
        raise CompositeError("A3 runner must contain exactly one standalone M0")
    ...
    if motion.search(executable):
        raise CompositeError("A3 runner has axis/rotary/probe motion before M0")
    ...
    for token in required_once:
        if text.count(token) != 1:
            raise CompositeError(f"A3 runner identity token changed: {token}")
    if re.search(r"\b(?:setp|halcmd)\b", text, re.IGNORECASE):
        raise CompositeError("A3 runner contains a HAL/coefficient write command")
    for forbidden in (
        ...
        if forbidden in text:
            raise CompositeError(f"A3 runner contains forbidden source/write target: {forbidden}")
    ...
    if len(log_paths) != 5 or set(log_paths) != expected_logs:
        raise CompositeError("A3 runner does not write exactly its five fresh outputs")
    ...
    if match is None:
        raise CompositeError("A3 mode-31 body is missing")
    body = match.group(1)
    ordered = (
        "#726 = 22.0",
    ...
    if any(position < 0 for position in positions) or positions != sorted(positions):
        raise CompositeError("A3 sequence 23-31 pose-block order changed")
    if body.count("o<tcpc_measure_pose> call") != 4:
        raise CompositeError("A3 body direct pose-call count changed")
    if body.count("o<tcpc_primary_tilt_block> call") != 0:
        raise CompositeError("A3 body unexpectedly invokes a full tilt block")
    if body.count("o<tcpc_recovery_closing_b0_sweep> call") != 1:
        raise CompositeError("A3 body closing-sweep call count changed")

    closure_token = "[#728] [#<sweep_open_seq>] [#726]"
    if text.count(closure_token) != 1:
        raise CompositeError("A3 source-local closure implementation changed")
    for forbidden_closure in (
        "[906.0] [16.0] [#726]",
        "[#728] [#<block_open_seq>] [#726]",
    ):
        if forbidden_closure in text:
            raise CompositeError("A3 contains a non-local closure implementation")
    ...
    for opening, closing, label in balances:
        if len(re.findall(opening, text, re.MULTILINE)) != len(
            ...
            raise CompositeError(f"A3 runner has unbalanced O-word {label} blocks")


def validate_a3_headers() -> None:
    contracts = {
        A3_RESULTS: anchor.RESULT_FIELDS,
        A3_STATE: anchor.STATE_FIELDS,
        A3_CLOSURES: campaign.CLOSURE_FIELDS,
        A3_TRACE: a5.TRACE_FIELDS,
        A3_GAP_TRACE: a5.GAP_TRACE_FIELDS,
    }
    for path, fields in contracts.items():
        ...
        if rows != [list(fields)]:
            raise CompositeError(f"A3 output is not the exact fresh header: {path}")


def validate_a3_complete() -> SegmentValidation:
    with validator_context(A3_MODE, A3_ATTEMPT, A3_SEQUENCES, A3_TRACE, A3_GAP_TRACE):
        spec = run_spec(
            "T3 exploratory attempt 3 recovery",
            A3_MODE,
            A3_RESULTS,
            A3_STATE,
            A3_CLOSURES,
        ...
        if accepted != list(A3_SEQUENCES):
            raise CompositeError("A3 accepted rows are not exactly seq23-31")
        gap_count, gap_terminal, terminal_gap_key = a5.validate_gap_trace(
            complete=True
        )
        contact_count, bursts, contact_terminal = a5.validate_contact_trace(
        ...
        if (
            ...
            raise CompositeError("completed A3 contains a terminal electrical fault")
        if contact_count != gap_count:
            raise CompositeError("completed A3 contact/gap row counts differ")
        trace_rows = a5.read_identity_rows(
            A3_TRACE, a5.TRACE_FIELDS, A3_MODE, A3_ATTEMPT
        )
        result_rows = a5.read_identity_rows(
            A3_RESULTS, anchor.RESULT_FIELDS, A3_MODE, A3_ATTEMPT
        )
        center_deltas = pass_center_deltas(result_rows, A3_SEQUENCES)
        rejected, rejected_transactions = retry_trace_summary(trace_rows)
        if contact_count != 72 + rejected_transactions:
            raise CompositeError("A3 trace count does not match its validated retry shapes")
    return SegmentValidation(
        centers,
        center_deltas,
        closure_norms,
...
def compose_centers(
    ...
    if set(a3_centers) != set(A3_SEQUENCES):
        raise CompositeError("A3 composite ownership changed")
    source_sets = (set(a1_centers), set(a2_centers), set(a3_centers))
    if any(
        source_sets[left] & source_sets[right]
        for left, right in ((0, 1), (0, 2), (1, 2))
    ):
        raise CompositeError("composite sequence sources overlap")
    combined = {**a1_centers, **a2_centers, **a3_centers}
...
def source_identity(seq: int) -> str:
    ...
    if seq in A3_SEQUENCES:
        return "mode31/attempt3"
    raise CompositeError(f"sequence {seq} has no acquisition source")


def validate_closure_partition() -> None:
    local = A1_LOCAL_CLOSURES + A2_LOCAL_CLOSURES + A3_LOCAL_CLOSURES
...
def write_preflight_report(
    path: Path, a1: SegmentValidation, a2: SegmentValidation
) -> None:
    lines = [
        ...
        f"- campaign/mode/attempt: `{CAMPAIGN}/{A3_MODE}/{A3_ATTEMPT}`",
        f"- recovery runner SHA-256: `{sha256(A3_PROGRAM)}`",
...
def write_result_report(
    ...
    local_norms = np.concatenate(
        (a1.closure_norms, a2.closure_norms, a3.closure_norms)
    )
    t3_pass_deltas = {
        **a1.pass_center_deltas,
        **a2.pass_center_deltas,
        **a3.pass_center_deltas,
    }
    ...
    a3_delta_values = np.array(list(a3.pass_center_deltas.values()))
    ...
    lines = [
        "# T3 R2-Transfer Three-Source Composite Report",
        "",
        "## R2 NOT ACCEPTED",
        "",
        f"- exploratory classification: `{score.classification}`",
        "- direct composite: `A1 seq1-14 + A2 seq15-22 + A3 seq23-31; no source alignment`",
        "- composite result/state rows: `31 / 31`",
        "- formal same-acquisition `31 / 31 / 14`: `NOT SATISFIED`",
        "- reason: accepted rows span mode30/attempt1, mode31/attempt2, and mode31/attempt3; only four closures are validated within one source",
        ...
        f"- A3 contact/gap rows: `{a3.contact_rows} / {a3.gap_rows}`",
        f"- A3 complete rejected try-1 poses: `{a3.rejected_try1_poses}`",
    ...
    for closure in A1_LOCAL_CLOSURES + A2_LOCAL_CLOSURES + A3_LOCAL_CLOSURES:
        lines.append(
    ...
    lines += [
        ...
        f"- T3 A3 seq23-31 min/max: `{float(np.min(a3_delta_values)):.6f} / {float(np.max(a3_delta_values)):.6f} mm`",
    ...
    lines += [
        ...
        "centers and C0 closing seq26 are A3. D(+45,C) and D(+90,C) are",
    ...
    lines += [
        ...
        "D(-45,C) spans A1/A2 and D(-90,C) spans A2/A3. No source translation",
        "is applied.",
        "",
    ...
    lines += [
        ...
        "mixed-source: A2 supplies B-45/C0 and B0/C0 seq16; A3 supplies the closing",
        "B0 sweep, while the B0/C0 recovery value averages A2 and A3 occurrences.",
...
def preflight(report: Path) -> None:
    ...
    validate_a3_program()
    validate_a3_headers()
    write_preflight_report(report, a1, a2)
...
def analyze(report: Path) -> None:
    ...
    a2 = validate_a2_partial_forensics()
    validate_a3_program()
    a3 = validate_a3_complete()
    centers = compose_centers(a1.centers, a2.centers, a3.centers)
    ...
    write_result_report(report, a1, a2, a3, centers, score, sensitivity, t4)
...
def self_test() -> None:
    ...
    if A1_SEQUENCES + A2_SEQUENCES + A3_SEQUENCES != ALL_SEQUENCES:
        raise CompositeError("A1/A2/A3 sequence ownership is not an exact partition")
    ...
    composed = compose_centers(
        ...
        {seq: synthetic[seq] for seq in A3_SEQUENCES},
    )
    ...
    for seq in A2_SEQUENCES + A3_SEQUENCES:
    ...
    a3_translation = np.array([-0.30, 0.60, -1.20])
    boundary_shifted = {}
    for seq, center in synthetic.items():
        if seq in A2_SEQUENCES:
            boundary_shifted[seq] = center + a2_translation
        elif seq in A3_SEQUENCES:
            boundary_shifted[seq] = center + a3_translation
        else:
    ...
    for key in base_contrasts:
        if key[0] == -45:
            ...
            expected_change = (a3_translation - a2_translation) / 2.0
        else:
    ...
    for key in base_paired:
        if key[0] == 45:
            ...
            expected_even_change = (a3_translation - a2_translation) / 4.0
            expected_odd_change = -(a3_translation - a2_translation) / 4.0
        if (
            np.linalg.norm(
    ...
    for sign in (1, -1):
        for c_deg in CONTRAST_C_ANGLES:
            ...
            expected_change = a3_translation / 2.0 if sign < 0 else np.zeros(3)
            if (
                np.linalg.norm(
...
def main() -> int:
    ...
    try:
        if args.self_test:
            self_test()
            print("T3 attempt-3 three-source analyzer self-test: PASS")
            return 0
        if args.preflight:
            preflight(args.preflight_report)
            print("T3 attempt-3 recovery static preflight: PASS")
        ...
        print(f"T3 attempt-3 composite validation: FAIL: {exc}", file=sys.stderr)
        return 1
